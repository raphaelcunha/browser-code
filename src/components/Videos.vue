...
export default {
  ...
  methods: {
      ...
      youtube
        .get(
          `search?part=snippet&q=${
            this.languageSelected.videos
          }&maxResults=30&type=playlist&order=relevance&key=${this.key}`
        )
        .then(response => {
    ...
    getVideo(item) {
      ...
      youtube
        .get(
          `playlistItems?part=snippet&maxResults=12&playlistId=${
            item.id.playlistId
          }&key=${this.key}`
        )
        .then(response => {
  ...
  <div class="repository col-md-12 row">
    ...
    <div v-if="videoSelected" class="mt30">
      ...
      <div class="col-md-3 col-xl-1">
        ...
        <!--//PLAYLIST-->
        <div class="card" v-for="item in videos" :key="item.snipper.title" @click="getVideo(item);">
          <div class="card-text">
      ...
      <div class="col-md-9" v-if="videoSelected">
        ...
        <!--//VIDEOS LIST-->
        <div class="col-md-4" v-for="item in playlist" :key="item.snippet.title" @click="videoSelected = item;">
